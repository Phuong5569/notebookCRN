...
from notebook.CRNNMdoels import *
from notebook.utils import create_new_book
from notebook.model_control import is_model_on, turn_on, turn_off, get_status
from datetime import date 
from django.utils.timezone import now
import shutil
import uuid
from notebook.segmentation import segment_book_lines
    ...
    try:
        ...
        try:
            ...
            if not os.path.exists(weights_path):
                return JsonResponse({'error': 'OCR model weights not found. Please place checkpoint.weights.hdf5 in your project root.'}, status=404)
        ...
        # save text context to file
        book_json_path = os.path.join(settings.BOOK_STORAGE_DIR, bookid, 'data.json')
        print(book_json_path)
        book_json = json.load(open(book_json_path, 'r'))
        book_json["context"] = full_text
        # print(book_json)
        ...
        if os.path.isfile(json_path):
            with open(json_path, "r", encoding="utf-8") as f:
                try:
                    filtered={}
                    book_data = json.load(f)
                    filtered = {key: book_data[key] for key in fields if key in book_data}
                    # print(filtered)
                    books.append(filtered)
                except Exception as e:
                    # print(e)
                    continue  # skip corrupted file
    return JsonResponse({"documents": books})


@csrf_exempt
@require_http_methods(["POST"])
def segment_lines_view(request):
    """
    API endpoint to segment all pages of a book into lines
    Saves in the exact format expected by extract_text_from_lines()
    """
    try:
        data = json.loads(request.body)
        ...
        return JsonResponse({
            ...
            'message': f"Successfully segmented {result['total_lines']} lines from {result['page_count']} pages"
        })
...
@csrf_exempt
def model_on(request):
    """Turn model ON - just access /notebook/model/on/"""
    message = turn_on()
    return HttpResponse(f"""
        ...
        <body style="font-family: Arial; padding: 50px; text-align: center;">
            ...
            <hr>
            <a href="/notebook/model/on/">🔄 Turn ON</a> | 
            <a href="/notebook/model/off/">💾 Turn OFF</a> | 
...
def model_off(request):
    """Turn model OFF - just access /notebook/model/off/"""
    message = turn_off()
    return HttpResponse(f"""
        <html>
        <body style="font-family: Arial; padding: 50px; text-align: center;">
...
def model_status(request):
    """Check status - just access /notebook/model/status/"""
    status = get_status()
    return HttpResponse(f"""
        <html>
        <body style="font-family: Arial; padding: 50px; text-align: center;">
            <h1>Model Status: {'ON ✅' if status else 'OFF ❌'}</h1>
            <p>Value: {status}</p>
            <hr>
            <a href="/notebook/model/on/">🔄 Turn ON</a> | 
            <a href="/notebook/model/off/">💾 Turn OFF</a> | 
